...
export default function ProfilePage() {
  ...
  const { data: userPrincipal, isLoading: principalLoading } =
    useGetPrincipalByUsername(username);
  const { data: userProfile, isLoading: profileLoading } = useGetUserProfile(
    userPrincipal ?? null,
  );
  const { data: posts = [], isLoading: postsLoading } = useGetVideoPostsByUser(
    userPrincipal ?? null,
  );
  const { data: replies = [], isLoading: repliesLoading } =
    useGetVideoRepliesByUser(userPrincipal ?? null);
  ...
  const avatarUrl = userProfile?.avatar
    ? URL.createObjectURL(
        new Blob([new Uint8Array(userProfile.avatar)], { type: "image/jpeg" }),
      )
    : undefined;
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            {!isOwnProfile && <FollowButton targetPrincipal={userPrincipal} />}
          </div>
