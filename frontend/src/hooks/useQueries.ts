...
import type { UserProfile, VideoPost, VideoReply, Notification, VisitorStats } from '../backend';
import { Principal } from '@dfinity/principal';

// Hardcoded permanent admin principal
const PERMANENT_ADMIN_PRINCIPAL = 'a7dxs-nk6zm-shch4-6mlx5-xd7vi-bqjwf-euhqd-d47mb-mtbhj-4o34v-2qe';
...
export function useGetRepliesForPostWithPagination(
  ...
  return useQuery<VideoReply[]>({
    queryKey: ['videoRepliesPaginated', videoId?.toString(), startIndex.toString(), batchSize.toString()],
    queryFn: async () => {
      if (!actor || videoId === null) return [];
      return actor.getRepliesForPostWithPagination(videoId, startIndex, batchSize);
    },
    enabled: !!actor && !actorFetching && videoId !== null,
  });
}
...
export function useCreateVideoReply() {
  ...
  return useMutation({
    mutationFn: async ({ parentVideoId, video }: { parentVideoId: bigint; video: any }) => {
      if (!actor) throw new Error('Actor not available');
      return actor.createVideoReply(parentVideoId, video);
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['videoReplies', variables.parentVideoId.toString()] });
      queryClient.invalidateQueries({ queryKey: ['videoRepliesPaginated', variables.parentVideoId.toString()] });
      queryClient.invalidateQueries({ queryKey: ['repliesCount', variables.parentVideoId.toString()] });
    },
  });
}
...
export function useLikeReply() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, replyId) => {
      queryClient.invalidateQueries({ queryKey: ['replyLikesCount', replyId.toString()] });
      queryClient.invalidateQueries({ queryKey: ['hasLikedReply', replyId.toString()] });
    },
  });
}
...
export function useFollowUser() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, targetPrincipal) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['followersAndFollowingCounts'] });
    },
  });
}
...
export function useGetFollowersAndFollowingCounts(userPrincipal: string | null) {
  const { actor, isFetching: actorFetching } = useActor();

  return useQuery<[bigint, bigint]>({
    queryKey: ['followersAndFollowingCounts', userPrincipal],
    queryFn: async () => {
      if (!actor || !userPrincipal) return [BigInt(0), BigInt(0)] as [bigint, bigint];
      return actor.getFollowersAndFollowingCounts(Principal.fromText(userPrincipal));
    },
    enabled: !!actor && !actorFetching && !!userPrincipal,
  });
}
...
export function useDeleteAccount() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['currentUserProfile'] });
      queryClient.invalidateQueries({ queryKey: ['accountDeletionTimeRemaining'] });
    },
  });
}
...
export function useGetUserAccountDeletionTimeRemaining(userPrincipal: string | null) {
  const { actor, isFetching: actorFetching } = useActor();
  ...
  return useQuery<bigint | null>({
    ...
    queryFn: async () => {
      if (!actor || !userPrincipal) return null;
      return actor.getUserAccountDeletionTimeRemaining(Principal.fromText(userPrincipal));
    },
    enabled: !!actor && !actorFetching && !!userPrincipal,
  });
}
