...
export default function VideoReplyCreator({ parentVideoId, onSuccess }: VideoReplyCreatorProps) {
  ...
  if (mode === 'select') {
    return (
      <div className="space-y-4 w-full min-w-0">
        ...
        <Button onClick={() => setMode('record')} variant="outline" className="w-full" size="lg">
          Record Video
        </Button>
      </div>
    );
        ...
        {videoFile && (
          <>
            ...
            <div className="flex gap-2 w-full min-w-0">
              <Button
                variant="outline"
                onClick={() => { setMode('select'); setVideoFile(null); }}
                disabled={uploading}
                className="flex-1 min-w-0"
              >
                Back
              </Button>
              <Button onClick={handleSubmit} disabled={uploading} className="flex-1 min-w-0">
