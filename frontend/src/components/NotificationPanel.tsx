...
import { useGetNotifications, useMarkNotificationsAsRead, useGetUserProfile } from '../hooks/useQueries';
...
function NotificationItem({
  ...
  const getNotificationText = (senderUsername: string | null) => {
    ...
    switch (notification.notificationType.__kind__) {
      ...
      default:
        return { prefix: '', username: 'Someone', suffix: ' sent you a notification.' };
    }
  };
  ...
  const handleNotificationClick = () => {
    navigate({ to: '/video/$videoId', params: { videoId: videoId.toString() } });
    onClose();
  };
  ...
  const handleUsernameClick = (e: React.MouseEvent) => {
    ...
    if (senderProfile?.username) {
      navigate({ to: '/profile/$username', params: { username: senderProfile.username } });
      onClose();
    }
  };
