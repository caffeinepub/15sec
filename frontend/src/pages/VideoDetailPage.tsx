...
export default function VideoDetailPage() {
  ...
  const { data: post, isLoading: postLoading } = useGetVideoPostById(videoIdBigInt);
  const { data: replies, isLoading: repliesLoading } = useGetRepliesForPostWithPagination(
    videoIdBigInt,
    BigInt(0),
    BigInt(loadedReplies)
  );
  const { data: totalRepliesCount } = useGetRepliesCountForPost(post?.id ?? null);
  ...
  const hasMoreReplies =
    totalRepliesCount !== undefined && replies ? Number(totalRepliesCount) > replies.length : false;
