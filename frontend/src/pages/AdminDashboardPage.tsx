...
import { Copy, UserPlus, UserMinus, Trash2 } from 'lucide-react';
...
function UserRow({
  ...
  principal: Principal;
  onAction: (principal: string, action: 'promote' | 'demote' | 'delete') => void;
}) {
    ...
    <TableRow>
      ...
      <TableCell>
        <div className="flex items-center gap-2">
          <span className="font-mono text-xs truncate max-w-[200px]">{principalStr}</span>
          <Button
            variant="ghost"
            size="icon"
...
export default function AdminDashboardPage() {
  ...
  const { data: visitorStats, isLoading: visitorStatsLoading } = useGetVisitorStats();
  ...
  const openConfirmDialog = (principal: string, action: 'promote' | 'demote' | 'delete') => {
    ...
    <div className="container max-w-7xl mx-auto p-4 pt-20 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {activeAccountsCount !== undefined ? activeAccountsCount.toString() : '0'}
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{allUsers ? allUsers.length : '0'}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-muted-foreground">Weekly Visitors</CardTitle>
            </CardHeader>
            <CardContent>
              {visitorStatsLoading ? (
                <Skeleton className="h-8 w-16" />
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-muted-foreground">All-Time Visitors</CardTitle>
            </CardHeader>
            <CardContent>
              {visitorStatsLoading ? (
                <Skeleton className="h-8 w-16" />
      ...
      <AlertDialog open={!!selectedUser} onOpenChange={(open) => { if (!open) { setSelectedUser(null); setActionType(null); } }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirm Action</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to {actionType ? actionLabels[actionType] : 'perform this action'}? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
