...
import { Sun, Moon, LogOut, Trash2, ShieldCheck, Loader2 } from 'lucide-react';
...
export default function AccountPage() {
  ...
  const { data: posts = [] } = useGetVideoPostsByUser(identity?.getPrincipal().toString() ?? null);
  const { data: replies = [] } = useGetVideoRepliesByUser(identity?.getPrincipal().toString() ?? null);
  ...
  const getAvatarUrl = () => {
    ...
    const blob = new Blob([new Uint8Array(userProfile.avatar)], { type: 'image/jpeg' });
    return URL.createObjectURL(blob);
  };

  const sortedPosts = [...posts].sort((a, b) => Number(b.timestamp - a.timestamp));
  const sortedReplies = [...replies].sort((a, b) => Number(b.timestamp - a.timestamp));
    ...
    <main className="max-w-2xl mx-auto px-4 py-8">
          ...
          <div className="flex-1 min-w-0">
            {editingUsername ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {saveProfile.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Save'}
                  </Button>
                  <Button
                    size="sm"
                    variant="ghost"
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="font-semibold text-lg truncate">{userProfile?.username}</h2>
                  {isAdmin && (
            ...
            <div className="flex gap-4 mt-3">
              <div className="text-center">
                <p className="font-bold text-base leading-tight">{followersCount}</p>
                <p className="text-xs text-muted-foreground">Followers</p>
              </div>
              <div className="text-center">
                <p className="font-bold text-base leading-tight">{followingCount}</p>
        ...
        {deletionTimeRemaining && (
          <div className="mt-4 p-3 bg-destructive/10 border border-destructive/20 rounded-md">
            <p className="text-sm text-destructive">
              Account scheduled for deletion in {formatDeletionTime(deletionTimeRemaining)}.
            </p>
            <Button
              size="sm"
              variant="outline"
            ...
            >
              {cancelDeletion.isPending ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : null}
              Cancel Deletion
            </Button>
          </div>
        )}
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Your account will be scheduled for deletion in 14 days. You can cancel this within that period. After 14 days, all your data will be permanently removed.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
      ...
      <Tabs defaultValue="posts">
        <TabsList className="grid w-full grid-cols-2">
          ...
          <TabsTrigger value="replies">Replies ({sortedReplies.length})</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="replies" className="space-y-6 mt-6">
          {sortedReplies.length === 0 ? (
            ...
            sortedReplies.map((reply) => (
              <VideoReplyCard key={reply.id.toString()} reply={reply} showDelete />
            ))
          )}
        </TabsContent>
      </Tabs>
